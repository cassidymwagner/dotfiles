...
document.getElementsByTagName('body')[0].addEventListener("click", function (el) {
    if (document.getElementById('menu').style.display == "block" && ["menuImg", "menu_button", "menu", "menu_inner"].indexOf(el.srcElement.id) == -1)
        document.getElementById('menu').style.display = "none";
});

document.body.addEventListener('click', e => {

    if (e && e.target && e.target.className && e.target.className.match('style-name')) {
        chrome.tabs.create({url: e.target.attributes[1].value});
