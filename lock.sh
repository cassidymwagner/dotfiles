...
# Original Image
orig_wall=$HOME/pictures/wallpapers/cozy_mountains.jpg
...
lock() {
	#$1 image path
	letterEnteredColor=8fa1b3ff
	letterRemovedColor=b48eadff
	passwordCorrect=a3be8cff
	passwordIncorrect=bf616aff
	background=2b303bff
	foreground=c0c5ceff
	i3lock \
		-n -i "$1" \
		...
		--datepos="tx-13:ty+25" \
		--clock --datestr "%A, %m %Y" \
		--insidecolor=$foreground --ringcolor=$background --line-uses-inside \
		--keyhlcolor=$letterEnteredColor --bshlcolor=$letterRemovedColor --separatorcolor=$background \
		--insidevercolor=$passwordCorrect --insidewrongcolor=$passwordIncorrect \
		--ringvercolor=$foreground --ringwrongcolor=$foreground --indpos="x+280:h-70" \
		--radius=15 --ring-width=8 --veriftext="" --wrongtext="" \
		--textcolor="$foreground" --timecolor="$foreground" --datecolor="$foreground" \
		--timefont="sourcecodepro" --datefont="sourcecodepro"
}
